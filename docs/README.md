# GA4: a practical guide for content designers

## About this guide
This guide explains how content designers can use GA4 (Google Analytics 4) to explore user behaviour and inform content decisions.

It focuses on:

- navigating the GA4 interface
- building explorations
- answering common content questions

The examples assume a standard (free-tier) GA4 setup without additional custom tracking. Where enhanced configurations provide additional options, this is noted separately.

Always follow your organisation's data protection and analytics policies.

No prior GA4 experience is required.
